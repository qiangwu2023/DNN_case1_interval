...
Set_lr = np.array([3.4e-4, 4e-4, 5e-4]) # learning rate
Beta = 1 # the true value of beta

# DNN: hyper-parameters (compute variance)
node_D = np.array([40, 35, 35]) 
lr_D = np.array([5e-4, 3e-4, 3e-4])

node_L = np.array([50, 50, 50]) 
lr_L = np.array([1e-3, 1e-3, 1e-3])

node_A = np.array([40, 35, 42]) 
lr_A = np.array([5e-4, 4e-4, 4e-4])

B = 200 # repeat times
#%% ------ generate testing data (sample size 200)------
test_data = generate_case_2(200, corr, Beta)
X_test = test_data['X']
...
ax1_1.set_title("Case 2, n=500",fontsize=10) # Set title and size
...
plt.ylim(-2,2) # Set vertical coordinate scale
ax1_2.set_title("Case 2, n=1000",fontsize=10) # Set title and size
...
plt.ylim(-2,2) # Set vertical coordinate scale
ax1_3.set_title("Case 2, n=2000",fontsize=10) # Set title and size
...
ax2_1.set_title("Case 2, n=500", fontsize=10) # Set title and size
...
plt.ylim(0,1) # Set vertical coordinate scale
ax2_2.set_title("Case 2, n=1000", fontsize=10) # Set title and size
...
plt.ylim(0,1) # Set vertical coordinate scale
ax2_3.set_title("Case 2, n=2000", fontsize=10) # Set title and size
...
for i in range(len(Set_n)):
    ...
    for b in range(B):
        ...
        train_data = generate_case_2(n, corr, Beta)
...
result_error.to_csv('result_error_additive_t.csv')
...
result_re.to_csv('result_re_additive_t.csv')
