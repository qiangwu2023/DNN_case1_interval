...
Set_n = np.array([500, 1000, 2000]) # three sample sizes
corr = 0.5 # correlation coefficient
n_layer = 3 # the number of layers
Set_node = np.array([50, 50, 50]) # layer-width
n_epoch = 200 # the number of epochs
Set_lr = np.array([2.2e-4, 3e-4, 5e-4]) # learning rate
Beta = 1 # the true value of beta

# DNN: hyper-parameters (compute variance)
node_D = np.array([30, 32, 40]) 
lr_D = np.array([4e-4, 4e-4, 4e-4])

node_L = np.array([45, 45, 50]) 
lr_L = np.array([1e-3, 1e-3, 1e-3])

node_A = np.array([40, 40, 40]) 
lr_A = np.array([2e-4, 2e-4, 2e-4])

B = 200 # repeat times
#%% ------ generate testing data (sample size 200)------
test_data = generate_case_4(200, corr, Beta)
X_test = test_data['X']
...
ax1_1.set_title("Case 4, n=500",fontsize=10) # Set title and size
...
plt.ylim(-2,2) # Set vertical coordinate scale
ax1_2.set_title("Case 4, n=1000",fontsize=10) # Set title and size
...
plt.ylim(-2,2) # Set vertical coordinate scale
ax1_3.set_title("Case 4, n=2000",fontsize=10) # Set title and size
ax1_3.set_xlabel("Predictor",fontsize=8) # Set horizontal coordinate label and size
# ax1_3.set_ylabel("Error",fontsize=8) # Set vertical coordinate label and size
ax1_3.tick_params(axis='both',labelsize=6) # Axis scale size
plt.subplots_adjust(left=None,bottom=None,right=None,top=None,wspace=0.3,hspace=0.15)
...
ax2_1.set_title("Case 4, n=500", fontsize=10) # Set title and size
...
plt.ylim(0,1) # Set vertical coordinate scale
ax2_2.set_title("Case 4, n=1000", fontsize=10) # Set title and size
...
plt.ylim(0,1) # Set vertical coordinate scale
ax2_3.set_title("Case 4, n=2000", fontsize=10) # Set title and size
ax2_3.set_xlabel("Time",fontsize=8) # Set horizontal coordinate label and size
# ax2_3.set_ylabel("Cumulative hazard function",fontsize=8) # Set vertical coordinate label and size
ax2_3.tick_params(axis='both',labelsize=6) # Axis scale size
ax2_3.plot(u_value, Lambda_true, color='k', label='True')
ax2_3.legend(loc='upper left', fontsize=6) # Display the position and size of the label
plt.subplots_adjust(left=None,bottom=None,right=None,top=None,wspace=0.3,hspace=0.15)
...
for i in range(len(Set_n)):
    ...
    for b in range(B):
        ...
        train_data = generate_case_4(n, corr, Beta)
...
result_error.to_csv('result_error_deep2_t.csv')
...
result_re.to_csv('result_re_deep2_t.csv')
